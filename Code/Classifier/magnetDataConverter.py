...
class MagnetProcessing():

	def __init__(self):
		self.dataSet = None; # will contain the dataSet of Sound

	def MagnetReadAndConvert(self):
...
def main():
	# try:
	obj = MagnetProcessing();
	magnetFile = 'data/magnet_data/magnet_data.txt';
	obj.run(magnetFile)

	# save the data to a file
	np.savetxt('magnet_data.txt',obj.dataSet,fmt='%.3f',delimiter=",");
