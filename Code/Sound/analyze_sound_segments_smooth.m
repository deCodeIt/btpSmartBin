...
for audioFilesEmpty = filesEmpty
    for audioFilesBook = filesBook
        for audioFilesWater = filesWater
            ...
            plot(xb,syb,'r','DisplayName','Book')
            plot(xw,syw,'b','DisplayName','Water');
        
            ylim([0,0.3]);
            legend('show');
            
            saveas(gcf,sprintf('Envelope_Analysis_Of_%s_%s_and_%s.tif',audioFilesEmpty{1},audioFilesBook{1},audioFilesWater{1}),'tiffn');
